...
These are files in this repo, so they render as long as GitHub renders.

    python scripts/cards.py --user parsaforughi --out assets
...
THEMES = {
    "dark": {
        "bg": "#0a0a0a", "border": "#2c2c2c", "title": "#39d353",
        "text": "#f3f2ee", "muted": "#8a8a86", "value": "#f3f2ee",
        "accent": "#39d353",
    },
    "light": {
...
def render_repo(repo, theme):
    ...
    pad = 18
    out = []
    name = repo.get("title") or repo["name"]
    ...
    return frame(W, H, c, "".join(out), f'{name} repository card')
...
def main(argv=None):
    ...
    for entry in wanted:
        ...
        card = {
            "name": src["name"],
            "title": entry.get("title") or src["name"],
